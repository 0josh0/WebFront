...
<%@ page
    import="org.iscas.databean.MarketSummaryDataBean, org.iscas.databean.QuoteDataBean,net.sf.json.JSONObject,org.codehaus.jackson.map.ObjectMapper"
    session="true" isThreadSafe="true" isErrorPage="false"%>
<%@ page import="org.iscas.service.TradeService" %>
<%@ page import="org.iscas.util.FinancialUtils" %>
...
MarketSummaryDataBean marketSummaryData = tradeService.getMarketSummary(uuid);
%>
...
while (gainers.hasNext() && (count++ < 5))
{
    ...
    QuoteDataBean quoteData = accountObjectMapper.readValue(jsonObj.toString(),QuoteDataBean.class);
%>
                    <TR align="center">
                        <TD width="24"><%= FinancialUtils.printQuoteLink(quoteData.getSymbol()) %>
                        </TD>
                        <TD><%= quoteData.getCurrentPrice() %></TD>
                        <TD width="52" nowrap><%= FinancialUtils.printGainHTML(quoteData.getCurrentPrice().subtract(quoteData.getOpenPrice())) %></TD>
                    </TR>
                    <%
}
%>
...
{
	//QuoteDataBean quoteData = (QuoteDataBean) gainers.next();
    JSONObject jsonObj= JSONObject.fromObject(losers.next());
    //System.out.println(jsonObj.toString());
    //将返回的json封装成account对象
    ObjectMapper accountObjectMapper = new ObjectMapper();
    QuoteDataBean quoteData = accountObjectMapper.readValue(jsonObj.toString(),QuoteDataBean.class);
%>
                    <TR align="center">
                        ...
                        <TD><%= quoteData.getCurrentPrice() %></TD>
                        <TD width="52" nowrap><%= FinancialUtils.printGainHTML(quoteData.getCurrentPrice().subtract(quoteData.getOpenPrice())) /* FinancialUtils.printGainPercentHTML(FinancialUtils.computeGainPercent(quoteData.getPrice(), quoteData.getOpen())) */%></TD>
                    </TR>
                    <%
}
%>
